...
class GenerationMixin:
    ...
    def generate(
        self,
        inputs: Optional[torch.Tensor] = None,
        inputs_relevant: Optional[torch.Tensor] = None,
        inputs_irrelevant: Optional[torch.Tensor] = None,
        ...
        **kwargs,
        ...
        Parameters:
            ...
            inputs_relevant (`torch.Tensor` of varying shape depending on the modality, *optional*):
                The (relevant) sequence used as a prompt for the generation or as model inputs to the encoder. If `None` the
                method initializes it with `bos_token_id` and a batch size of 1. For decoder-only models `inputs`
                should of in the format of `input_ids`. For encoder-decoder models *inputs* can represent any of
                `input_ids`, `input_values`, `input_features`, or `pixel_values`.
            inputs_irrelevant (`torch.Tensor` of varying shape depending on the modality, *optional*):
                The (irrelevant) sequence used as a prompt for the generation or as model inputs to the encoder. If `None` the
                method initializes it with `bos_token_id` and a batch size of 1. For decoder-only models `inputs`
                should of in the format of `input_ids`. For encoder-decoder models *inputs* can represent any of
                `input_ids`, `input_values`, `input_features`, or `pixel_values`.
            ...
            alpha_weight (`float`, *optional*):
                The weight used to aggregate the logits distribution for decoding; the value should be larger than 0.
        ...
        if inputs_relevant is not None: model_kwargs_relevant = model_kwargs.copy()
        if inputs_irrelevant is not None: model_kwargs_irrelevant = model_kwargs.copy()
        ...
        if inputs_relevant is not None:
            inputs_tensor_relevant, _, model_kwargs_relevant = self._prepare_model_inputs(
                inputs_relevant, generation_config.bos_token_id, model_kwargs_relevant
            )
        if inputs_irrelevant is not None:
            inputs_tensor_irrelevant, _, model_kwargs_irrelevant = self._prepare_model_inputs(
                inputs_irrelevant, generation_config.bos_token_id, model_kwargs_irrelevant
            )
        ...
        # model_kwargs_relevant["output_attentions"] = generation_config.output_attentions
        # model_kwargs_relevant["output_hidden_states"] = generation_config.output_hidden_states
        # model_kwargs_irrelevant["output_attentions"] = generation_config.output_attentions
        # model_kwargs_irrelevant["output_hidden_states"] = generation_config.output_hidden_states

        # decoder-only models with inputs_embeds forwarding must use caching (otherwise we can't detect whether we are
        # generating the first new token or not, and we only want to use the embeddings for the first new token)
        if not self.config.is_encoder_decoder and model_input_name == "inputs_embeds":
            model_kwargs["use_cache"] = True
            if inputs_relevant is not None: model_kwargs_relevant["use_cache"] = True
            if inputs_irrelevant is not None: model_kwargs_irrelevant["use_cache"] = True
        else:
            model_kwargs["use_cache"] = generation_config.use_cache
            if inputs_relevant is not None: model_kwargs_relevant["use_cache"] = generation_config.use_cache
            if inputs_irrelevant is not None: model_kwargs_irrelevant["use_cache"] = generation_config.use_cache
        ...
        if model_kwargs.get("attention_mask", None) is None and requires_attention_mask and accepts_attention_mask:
            ...
            if inputs_relevant is not None:
                model_kwargs_relevant["attention_mask"] = self._prepare_attention_mask_for_generation(
                    inputs_tensor_relevant, generation_config.pad_token_id, generation_config.eos_token_id
                )
            if inputs_irrelevant is not None:
                model_kwargs_irrelevant["attention_mask"] = self._prepare_attention_mask_for_generation(
                    inputs_tensor_irrelevant, generation_config.pad_token_id, generation_config.eos_token_id
                )
        ...
        if self.config.is_encoder_decoder and "encoder_outputs" not in model_kwargs:
            ...
            if inputs_relevant is not None:
                model_kwargs_relevant = self._prepare_encoder_decoder_kwargs_for_generation(
                    inputs_tensor_relevant, model_kwargs_relevant, model_input_name
                )
            if inputs_irrelevant is not None:
                model_kwargs_irrelevant = self._prepare_encoder_decoder_kwargs_for_generation(
                    inputs_tensor_irrelevant, model_kwargs_irrelevant, model_input_name
                )
        ...
        if self.config.is_encoder_decoder:
            ...
            if inputs_relevant is not None:
                input_ids_relevant, model_kwargs_relevant = self._prepare_decoder_input_ids_for_generation(
                    batch_size=batch_size,
                    model_input_name=model_input_name,
                    model_kwargs=model_kwargs_relevant,
                    decoder_start_token_id=generation_config.decoder_start_token_id,
                    bos_token_id=generation_config.bos_token_id,
                    device=inputs_tensor_relevant.device,
                )
            if inputs_irrelevant is not None:
                input_ids_irrelevant, model_kwargs_irrelevant = self._prepare_decoder_input_ids_for_generation(
                    batch_size=batch_size,
                    model_input_name=model_input_name,
                    model_kwargs=model_kwargs_irrelevant,
                    decoder_start_token_id=generation_config.decoder_start_token_id,
                    bos_token_id=generation_config.bos_token_id,
                    device=inputs_tensor_irrelevant.device,
                )
        else:
            input_ids = inputs_tensor if model_input_name == "input_ids" else model_kwargs.pop("input_ids")
            if inputs_relevant is not None: input_ids_relevant = inputs_tensor_relevant if model_input_name == "input_ids" else model_kwargs_relevant.pop("input_ids")
            if inputs_irrelevant is not None: input_ids_irrelevant = inputs_tensor_irrelevant if model_input_name == "input_ids" else model_kwargs_irrelevant.pop("input_ids")

        if streamer is not None:
            streamer.put(input_ids.cpu())
            if inputs_relevant is not None: streamer.put(input_ids_relevant.cpu())
            if inputs_irrelevant is not None: streamer.put(input_ids_irrelevant.cpu())

        # 6. Prepare `max_length` depending on other stopping criteria.
        input_ids_length = input_ids.shape[-1]
        if inputs_relevant is not None: input_ids_length_relevant = input_ids_relevant.shape[-1]
        if inputs_irrelevant is not None: input_ids_length_irrelevant = input_ids_irrelevant.shape[-1]
        ...
        if inputs_relevant is not None: 
            logits_processor_relevant = self._get_logits_processor(
                generation_config=generation_config,
                input_ids_seq_length=input_ids_length_relevant,
                encoder_input_ids=inputs_tensor_relevant,
                prefix_allowed_tokens_fn=prefix_allowed_tokens_fn,
                logits_processor=logits_processor,
                model_kwargs=model_kwargs_relevant,
                negative_prompt_ids=negative_prompt_ids,
        ...
        if inputs_irrelevant is not None: 
            logits_processor_irrelevant = self._get_logits_processor(
                generation_config=generation_config,
                input_ids_seq_length=input_ids_length_irrelevant,
                encoder_input_ids=inputs_tensor_irrelevant,
                prefix_allowed_tokens_fn=prefix_allowed_tokens_fn,
                logits_processor=logits_processor,
                model_kwargs=model_kwargs_irrelevant,
                negative_prompt_ids=negative_prompt_ids,
        ...
        if generation_mode == GenerationMode.GREEDY_SEARCH:
            ...
            return self.greedy_search(
                input_ids,
                input_ids_relevant=input_ids_relevant if inputs_relevant is not None else None,
                input_ids_irrelevant=input_ids_irrelevant if inputs_irrelevant is not None else None,
                logits_processor=logits_processor,
                logits_processor_relevant=logits_processor_relevant if inputs_relevant is not None else None,
                logits_processor_irrelevant=logits_processor_irrelevant if inputs_irrelevant is not None else None,
                stopping_criteria=stopping_criteria,
                pad_token_id=generation_config.pad_token_id,
                eos_token_id=generation_config.eos_token_id,
                output_scores=generation_config.output_scores,
                ...
                synced_gpus=synced_gpus,
                streamer=streamer,
                model_kwargs_relevant=model_kwargs_relevant if inputs_relevant is not None else None,
                model_kwargs_irrelevant=model_kwargs_irrelevant if inputs_irrelevant is not None else None,
                alpha=alpha_weight,
                **model_kwargs,
            )
    ...
    def greedy_search(
        self,
        input_ids: torch.LongTensor,
        input_ids_relevant: Optional[torch.LongTensor] = None,
        input_ids_irrelevant: Optional[torch.LongTensor] = None,
        logits_processor: Optional[LogitsProcessorList] = None,
        logits_processor_relevant: Optional[LogitsProcessorList] = None,
        logits_processor_irrelevant: Optional[LogitsProcessorList] = None,
        stopping_criteria: Optional[StoppingCriteriaList] = None,
        ...
        synced_gpus: bool = False,
        streamer: Optional["BaseStreamer"] = None,
        model_kwargs_relevant: Optional[dict] = None,
        model_kwargs_irrelevant: Optional[dict] = None,
        ...
        **model_kwargs,
        ...
        Parameters:
            input_ids (`torch.LongTensor` of shape `(batch_size, sequence_length)`):
                The sequence used as a prompt for the generation.
            input_ids_relevant (`torch.LongTensor` of shape `(batch_size, sequence_length)`, *optional*):
                The (relevant) sequence used as a prompt for the generation.
            input_ids_irrelevant (`torch.LongTensor` of shape `(batch_size, sequence_length)`, *optional*):
                The (irrelevant) sequence used as a prompt for the generation.
            logits_processor (`LogitsProcessorList`, *optional*):
                An instance of [`LogitsProcessorList`]. List of instances of class derived from [`LogitsProcessor`]
                used to modify the prediction scores of the language modeling head applied at each generation step.
            logits_processor_relevant (`LogitsProcessorList`, *optional*):
                An instance of [`LogitsProcessorList`]. List of instances of class derived from [`LogitsProcessor`]
                used to modify the prediction scores of the language modeling head applied at each generation step.
            logits_processor_irrelevant (`LogitsProcessorList`, *optional*):
                An instance of [`LogitsProcessorList`]. List of instances of class derived from [`LogitsProcessor`]
                used to modify the prediction scores of the language modeling head applied at each generation step.
            stopping_criteria (`StoppingCriteriaList`, *optional*):
                An instance of [`StoppingCriteriaList`]. List of instances of class derived from [`StoppingCriteria`]
            ...
            streamer (`BaseStreamer`, *optional*):
                Streamer object that will be used to stream the generated sequences. Generated tokens are passed
                through `streamer.put(token_ids)` and the streamer is responsible for any further processing.
            model_kwargs_relevant (`dict`, *optional*):
                Additional model specific keyword arguments will be forwarded to the `forward` function of the model.
                If model is an encoder-decoder model the kwargs should include `encoder_outputs`.
            model_kwargs_irrelevant (`dict`, *optional*):
                Additional model specific keyword arguments will be forwarded to the `forward` function of the model.
                If model is an encoder-decoder model the kwargs should include `encoder_outputs`.
        ...
        # init values
        logits_processor = logits_processor if logits_processor is not None else LogitsProcessorList()
        if input_ids_relevant is not None: logits_processor_relevant = logits_processor_relevant if logits_processor_relevant is not None else LogitsProcessorList()
        if input_ids_irrelevant is not None: logits_processor_irrelevant = logits_processor_irrelevant if logits_processor_irrelevant is not None else LogitsProcessorList()
        stopping_criteria = stopping_criteria if stopping_criteria is not None else StoppingCriteriaList()
        ...
        alpha = torch.tensor(alpha).to(input_ids.device) if alpha is not None else None
        if input_ids_relevant is not None and alpha is None: 
            warnings.warn(
                "You are using custom decoding strategy and you have have not set a value for alpha.",
                UserWarning,
            )
        ...
        while True:
            ...
            model_inputs = self.prepare_inputs_for_generation(input_ids, **model_kwargs)
            if input_ids_relevant is not None: model_inputs_relevant = self.prepare_inputs_for_generation(input_ids_relevant, **model_kwargs_relevant)
            if input_ids_irrelevant is not None: model_inputs_irrelevant = self.prepare_inputs_for_generation(input_ids_irrelevant, **model_kwargs_irrelevant)
            ...
            if input_ids_relevant is not None: 
                outputs_relevant = self(
                    **model_inputs_relevant,
                    return_dict=True,
                    output_attentions=output_attentions,
                    output_hidden_states=output_hidden_states,
                )
            if input_ids_irrelevant is not None: 
                outputs_irrelevant = self(
                    **model_inputs_irrelevant,
                    return_dict=True,
                    output_attentions=output_attentions,
                    output_hidden_states=output_hidden_states,
                )
            ...
            if input_ids_relevant is not None: next_token_logits_relevant = outputs_relevant.logits[:, -1, :]
            if input_ids_irrelevant is not None: next_token_logits_irrelevant = outputs_irrelevant.logits[:, -1, :]

            # pre-process distribution
            next_tokens_scores = logits_processor(input_ids, next_token_logits)
            if input_ids_relevant is not None: next_tokens_scores_relevant = logits_processor_relevant(input_ids_relevant, next_token_logits_relevant)
            if input_ids_irrelevant is not None: next_tokens_scores_irrelevant = logits_processor_irrelevant(input_ids_irrelevant, next_token_logits_irrelevant)

            if input_ids_relevant is not None and input_ids_irrelevant is not None: 
                # our custom decoding
                next_tokens_scores = next_tokens_scores + alpha * (next_tokens_scores_relevant - next_tokens_scores_irrelevant)
            elif input_ids_relevant is not None and input_ids_irrelevant is None:
                # CAD decoding 
                next_tokens_scores = (1 + alpha) * next_tokens_scores_relevant - alpha * next_tokens_scores
            ...
            # update generated ids, model inputs, and length for next step
            input_ids = torch.cat([input_ids, next_tokens[:, None]], dim=-1)
            if input_ids_relevant is not None: input_ids_relevant = torch.cat([input_ids_relevant, next_tokens[:, None]], dim=-1)
            if input_ids_irrelevant is not None: input_ids_irrelevant = torch.cat([input_ids_irrelevant, next_tokens[:, None]], dim=-1)
            if streamer is not None:
                streamer.put(next_tokens.cpu())
            model_kwargs = self._update_model_kwargs_for_generation(
                outputs, model_kwargs, is_encoder_decoder=self.config.is_encoder_decoder
            ...
            if input_ids_relevant is not None: 
                model_kwargs_relevant = self._update_model_kwargs_for_generation(
                    outputs_relevant, model_kwargs_relevant, is_encoder_decoder=self.config.is_encoder_decoder
                )
            if input_ids_irrelevant is not None: 
                model_kwargs_irrelevant = self._update_model_kwargs_for_generation(
                    outputs_irrelevant, model_kwargs_irrelevant, is_encoder_decoder=self.config.is_encoder_decoder
                )
